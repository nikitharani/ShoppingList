...
const App = () => {
  ...
  const toggleCurrentDelete = (index) => {
    ...
    const newItems = current_items.filter(
      (item) => item.itemName !== deletedItem.itemName
    );
  ...
  const togglePreviousDelete = (index) => {
    ...
    const newItems = previous_items.filter(
      (item) => item.itemName !== deletedItem.itemName
    );
    setPreviousItems(newItems);
  };
      ...
      <div className="main-container">
                  ...
                  <>
                    <div className="sel-item">
                      <div>
                        <FontAwesomeIcon
                          icon={faCheckCircle}
                          onClick={() => toggleCurrentComplete(index)}
                        />
                        <span className="completed">
                          <ContentEditable
                            html={item.itemName}
                            onChange={(event) =>
                              toggleCurrentEdit(index, event.target.value)
                            }
                          />
                        </span>
                        <FontAwesomeIcon
                          icon={faTrash}
                          onClick={() => toggleCurrentDelete(index)}
                        />
                      </div>
                    </div>
                  </>
        ...
        <div class="option-btn">
          <Button title="Save List">
            {" "}
            <CSVLink data={current_items}>
              <FontAwesomeIcon icon={faSave} />
            </CSVLink>
          </Button>
          <Button title="Clear List" onClick={() => toggleClearList()}>
            {" "}
                  ...
                  <>
                    <div className="sel-item">
                      <div>
                        <FontAwesomeIcon
                          icon={faCheckCircle}
                          onClick={() => togglePreviousComplete(index)}
                        />
                        <span className="completed">
                          <ContentEditable
                            html={item.itemName}
                            onChange={(event) =>
                              togglePreviousEdit(index, event.target.value)
                            }
                          />
                        </span>
                        <FontAwesomeIcon
                          icon={faTrash}
                          onClick={() => togglePreviousDelete(index)}
                        />
                      </div>
                    </div>
                  </>
